...
class SpellFactory:
    ...
    def get_available_spells(piece) -> List[Spell]:
        ...
        if piece.type == "Warrior":
            ...
            available_spells.extend([
                ...
                or spell.effect_type in [SpellEffectType.DAMAGE, SpellEffectType.HEAL, SpellEffectType.DEBUFF]
            ])
        elif piece.type == "Archer":
            available_spells.extend([
                spell for spell in all_spells
                if spell.name in ["Arrow Hit"]
    ...
    @staticmethod
    def is_spell_available_for_piece(piece, spell: Spell) -> bool:
        """判断法术是否在该棋子职业可用列表中（与施法次数 spell_slots 无关）。"""
        if piece is None or spell is None:
            return False
        return any(s.id == spell.id for s in SpellFactory.get_available_spells(piece))

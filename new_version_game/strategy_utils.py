...
def simulate_attack(env: Environment, attacker: Piece, target: Piece) -> float:
    ...
    if getattr(attacker, "weapon_type", 0) == 4:
        return 4.0
    height_bonus = env.height_physical_damage_bonus(attacker, target)
    raw_damage = attacker.physical_damage + attacker.strength + height_bonus
...
def fork_environment(env: Environment) -> Environment:
    ...
    new_env.is_battle_initialized = env.is_battle_initialized
    new_env.max_rounds = env.max_rounds
    new_env.zone_radius = env.zone_radius
    ...
    if env.board:
        ...
        new_env.board.capture_cells = set(env.board.capture_cells)

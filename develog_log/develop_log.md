...
1. evaluation 的时候第一个人行动了两次
2. mcts 我们设想的是一场比赛一个人只建立一棵树，然后如果我们希望执行 action 动作，那么就走到对应节点。
3. 我们没有内存回收机制。内存占用过大，导致内存溢出
4. 我们的 current_piece 可能为 none，虽然我们已经加了补救方法，但这种补救方法正确性存疑。
...
1. 我需要让程序输出每次棋子的策略是什么？检查训练成果
2. 我需要询问 ai 为什么没有训练成功。

命令：

1. 我需要你修改好 model_battle.py 里面的 draw 的逻辑，他现在不输出 draw 的局面
2. 我需要你添加调试 api，让我能够实时检测到棋子的 actionset
3. 我之前对战的两个模型是训练了很久的模型和没训练的模型。它们仍然打成平手，说明根本没有训练效果。我之前一直都是在让模型与 aggressive 的行动策略，arch29的行为策略进行对战，我之前的训练代码出现了问题。请你帮助我改正。

命令：

1. 我们的评估函数似乎有问题，你需要修改一下。
2. 我现在需要你从0开始，不要有任何数据，与 以 arch29 为初始化策略，以 aggressive 为行动策略的 总体 policy 学习并训练。监视整个训练过程。调整评估函数使得 5 次训练评估一次。我需要看到我们的策略要渐渐的能赢过最初的自己。

命令：

1. 不用每个 epoch 都加一个进度条，有一个总的 200 epoch 的进度条就行。
2. 我们在 evaluation 的时候，加一个 step 的进度条。

运行：

python model_train.py --resume-model training_data\run_20260527_175343\iteration_3_model.pt

我们需要监视这一次训练的成果，即能不能打过之前的自己。评估的时候输出每个人的策略。

...
```bash
cd client/client
python local_client.py --mode local --board ./BoardCase/case2.txt
```
...
| `--board` | 棋盘文件路径 | 任意有效路径 | `./BoardCase/case2.txt` |
...
```python
# 初始化游戏
env.initialize(board_file: str = "./BoardCase/case2.txt") -> None
...
```python
# 运行完整游戏
env.run(board_file: str = "./BoardCase/case2.txt") -> None
...
# 按先攻生成行动队列（initialize / setup_battle_host 内部调用）
# 先攻 = roll_dice(1,5)+dexterity；平局用 1..N 洗牌编号升序打破
env._compute_initiative_order(pieces) -> List[Piece]
...
- 伤害类法术：`spell_damage = spell.base_value + caster.intelligence // 2`；治疗类按 `base_value` 恢复，不超过 `max_health`。
- 法术伤害/治疗**不**经过物理/魔法抗性；法术射程**不**受高度差影响。
...
> 完整规则见 **`GAME_RULES.md`**；相对初版差异见 **`BALANCE_CHANGELOG.md`**。

### 生命值
- `max_health = 80`（固定，与力量无关）

### 行动点
- `max_action_points = 2`（固定）
...
### 先手 / 行动顺序
- 先攻值 = `dexterity`，**越高越先行动**（`Environment._compute_initiative_order`）。
- 先攻相同：开战前将 `1..N`（`N` 为双方棋子总数，标准为 6）随机洗牌后各分配唯一编号；**编号较小者**更靠前。

### 移动力
- `max_movement = dexterity // 2 + 6`，再叠加防具修正（轻 +3 / 中 0 / 重 −3）

### 高地优势（仅物理普攻）
- `Δh = max(0, 攻方.height − 守方.height)`
- 有效射程 = `attack_range + 2×Δh`
- 物理伤害额外 `+ 3×Δh`（抗性前）；法术与法杖 4 点真实伤害不适用

### 死亡
- HP ≤ 0 立即死亡，无 d20 苟活检定

### 中央高地缩圈（case2 等 40×40 地图）
- 终点：中央区域 `height_map ≥ 1` 的格（case2：`x,y ∈ [18,21]`）
- `currentRound` ≤ 2 不缩圈；3–90 线性缩小；第 90 回合安全区仅剩中央高地
- 每完整行动轮：圈外棋子受到 **30** 点伤害（无视护甲）
- 状态 JSON 字段 `safeZone`：`centerX/Y`、`radius`、`shrinkGraceRounds`、`shrinkEndRound`、`outsideDamage`、`finalRadius`
